...
* Embedded Coherence
* Use a remote Coherence cache using https://docs.oracle.com/en/middleware/standalone/coherence/15.1.1/develop-remote-clients/introduction-coherenceextend.html[Coherence*Extend]
...
----
DEMO_MODULES=samples/coherence-hibernate-demo/coherence-hibernate-demo-app,samples/coherence-hibernate-demo/coherence-hibernate-demo-server
./mvnw clean package -DskipTests -pl "$DEMO_MODULES" -am
----

The executable JARs use stable names, so the commands below work unchanged from development checkouts and release tags.
...
----
java -jar samples/coherence-hibernate-demo/coherence-hibernate-demo-app/target/coherence-hibernate-demo-app.jar
----
...
[source,bash,indent=1,subs="verbatim,quotes,attributes"]
----
java -jar samples/coherence-hibernate-demo/coherence-hibernate-demo-server/target/coherence-hibernate-demo-server.jar
----
...
[source,bash,indent=1,subs="verbatim,quotes,attributes"]
----
java -jar samples/coherence-hibernate-demo/coherence-hibernate-demo-app/target/coherence-hibernate-demo-app.jar --spring.profiles.active=remote
----
...
In the console you should see some basic statistics being printed:
...
The application uses Spring Boot `4.1`, Hibernate ORM `7.4`, and Jakarta Persistence `3.2` on Java 17, 21, or 25.
...
Add the Hibernate 7 adapter to your `pom.xml`:
...
hibernate.cache.region.factory_class=com.oracle.coherence.hibernate.cache.v7.CoherenceRegionFactory
...
Furthermore, we have `2` REST controllers.

== EventController

* GET `/api/events` Gets a paginated list of events
* POST `/api/events?title=foo&date=2020-10-30` Create a single event
* GET `+/api/events/{id}+` Gets a single event; optional `usingJpaQuery` and `withParticipants` parameters control how it is loaded

== PersonController

* GET `/api/people` Gets a paginated list of people
* POST `/api/people?firstName=Eric&lastName=Cartman&age=10` Create a single person
* GET `+/api/people/{personId}+` Gets a single person
...
Uncomment the following sections for the server and the client module:
...
-jar samples/coherence-hibernate-demo/coherence-hibernate-demo-server/target/coherence-hibernate-demo-server.jar
----
...
----
java -DauthenticationToken=my_secret_token  \
-jar samples/coherence-hibernate-demo/coherence-hibernate-demo-app/target/coherence-hibernate-demo-app.jar \
--spring.profiles.active=remote
----

For more information, please consult the reference documentation on
https://docs.oracle.com/en/middleware/standalone/coherence/15.1.1/secure/securing-extend-client-connections.html[Using Identity Tokens to Restrict Client Connections]
