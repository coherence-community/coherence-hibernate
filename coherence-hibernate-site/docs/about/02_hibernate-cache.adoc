...
The default `4.x` build uses Hibernate ORM `7.4.5.Final`. CI brackets the supported `7.4.x` line by testing
`7.4.1.Final` and `7.4.5.Final` against each compatibility-tested Coherence version on Java 17. The complete default
...
`26.07`. Both versions are tested with Hibernate `7.4.1.Final` and `7.4.5.Final` on Java 17; the default
`15.1.1-0-5` and Hibernate `7.4.5.Final` combination receives the complete Java 17, 21, and 25 reactor validation.
