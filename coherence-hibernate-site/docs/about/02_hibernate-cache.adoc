...
The default Coherence Hibernate `4.x` build uses Coherence CE `15.1.1-0-2`. CI also compatibility-tests Coherence CE
`25.03.1`. Both versions are tested with Hibernate `7.4.1.Final` and `7.4.5.Final` on Java 17; the default
`15.1.1-0-2` and Hibernate `7.4.5.Final` combination receives the complete Java 17, 21, and 25 reactor validation.
...
Coherence Hibernate 4.x always uses Hibernate 7's `DefaultCacheKeysFactory`. The former
`hibernate.cache.keys_factory` setting is deprecated in Hibernate 7 and is not configurable by this provider. This
preserves tenant and entity metadata in cache keys and supports composite identifiers.
If this setting is explicitly configured, the provider logs a warning at startup; remove it from your configuration.
